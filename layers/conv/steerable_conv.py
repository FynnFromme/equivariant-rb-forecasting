...
class RBSteerableConv(enn.EquivariantModule):
    def __init__(self, 
        ...
        Args:
            ...
            v_share (int): The number of neighboring output-heights a kernel is shared with (in an continuous manner).
        ...
        assert v_share%2 == 1, 'v_share must be odd'
        assert v_share//2 < out_height, 'v_share is too large'

        groups = out_height + 2 * (v_share//2)
        ...
        r2_conv_in_type = FieldType(gspace, groups*v_kernel_size*in_fields) # concatenated neighborhoods
        r2_conv_out_type = FieldType(gspace, groups*out_fields)
        ...
        self.out_fields = v_share*out_fields
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = self.r2_conv.forward(input) # shape (batch*v_share, groups*fields, width, depth)
    ...
    def _concat_v_share_along_batch(self, tensor: Tensor) -> Tensor:
        ...
        Returns:
            Tensor: The transformed tensor of shape [batch*v_share, groups*ksize*sum(fieldsizes), width, depth].
        """
        ...
        tensor = F.pad(tensor, (*([0,0]*3), *(self.v_share-1, self.v_share-1))) # shape (b,padH,ksize*fields,w,d)
        
        tensor = tensor.unfold(dimension=1, size=self.v_share, step=1) # shape (b,groups,ksize*fields,w,d,vshare)
        
        tensor = tensor.permute(0, 5, 1, 2, 3, 4) # shape (b,vshare,groups,ksize*fields,w,d)
        tensor = tensor.flatten(start_dim=2, end_dim=3) # shape (b,vshare,groups*ksize*fields,w,d)
        tensor = tensor.flatten(start_dim=0, end_dim=1) # shape (b*vshare,groups*ksize*fields,w,d)
    ...
    def _split_v_share_along_batch(self, tensor: Tensor) -> Tensor:
        ...
        Args:
            tensor (Tensor): The tensor of shape [batch*v_share, groups*sum(fieldsizes), width, depth].

        Returns:
            Tensor: The transformed tensor of shape [batch, outHeight*vshare*sum(fieldsizes), width, depth].
        """
        ...
        tensor = tensor.reshape(batch_size, self.v_share, self.groups, -1, width, depth)
        tensor = tensor.flip(dims=[1])
        tensor = tensor.unfold(dimension=2, size=self.v_share, step=1)
        tensor = tensor.diagonal(offset=0, dim1=1, dim2=-1)
        tensor = tensor.permute(0, 1, 5, 2, 3, 4)
        tensor = tensor.flatten(1, 3)
